...
<head>
    ...
    <title>Report Download Requests</title>
...
<body>
    ...
    <div class="container">
        <div class="page-title">
            <h1>Report Downloads <span class="count">(<?php echo $total_count; ?>)</span></h1>
            <a href="<?php echo base_url('administrator/exportreportdownloads'); ?>" class="btn-export">
            ...
            <div class="card" data-content="<?php echo htmlspecialchars(strtolower(($c->name ?? '').($c->email ?? '').($c->message ?? ''))); ?>">
                <div class="card-header" onclick="toggleCard(this)">
                    <div class="contact-info">
                        <div class="avatar"><?php echo strtoupper(substr($c->name ?? 'R', 0, 1)); ?></div>
                    ...
                    <div class="meta">
                        ...
                        <a href="<?php echo base_url('administrator/deleteproduct/' . $c->id . '/id/contact_form/no-image/managereportdownloads'); ?>" onclick="return confirm('Are you sure you want to delete this report download request?'); event.stopPropagation();" style="color: #64748b; font-size: 13px; margin: 0 8px; transition: color 0.2s;" onmouseover="this.style.color='#ef4444'" onmouseout="this.style.color='#64748b'"><i class="fas fa-trash-alt"></i></a>
                        <i class="fas fa-chevron-down toggle-icon"></i>
                    </div>
                </div>
                <div class="card-body">
                    <div class="message-label">Details / Message</div>
                    <div class="message"><?php echo nl2br(htmlspecialchars($c->message ?? 'No details provided')); ?></div>
                </div>
            </div>
            <?php endforeach; else: ?>
            <div class="empty"><i class="fas fa-inbox"></i>No report downloads yet</div>
            <?php endif; ?>
        </div>
